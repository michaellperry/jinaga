...
var splitSegments = require('../node/query/segmenter');
...
describe('Query segmenter', function () {
    ...
    it('should accept a successor', function () {
        var segments = splitSegments(fromDescriptiveString('S.parent'));
        expect(segments.length).to.equal(1);
        expect(segments[0].toDescriptiveString()).to.equal('S.parent');
    });
    it('should accept a predecessor', function () {
        var segments = splitSegments(fromDescriptiveString('P.parent'));
        expect(segments.length).to.equal(0);
    });
    it('should accept a successor with a field', function () {
        var segments = splitSegments(fromDescriptiveString('S.parent F.type="Type"'));
        expect(segments.length).to.equal(1);
        expect(segments[0].toDescriptiveString()).to.equal('S.parent F.type="Type"');
    });
    it('should ignore leading fields', function () {
        var segments = splitSegments(fromDescriptiveString('F.type="Type" S.parent'));
        expect(segments.length).to.equal(1);
        expect(segments[0].toDescriptiveString()).to.equal('S.parent');
    });
    it('should accept two successors with fields', function () {
        var segments = splitSegments(fromDescriptiveString('S.grandParent F.type="TypeParent" S.parent F.type="TypeChild"'));
    ...
    it('should continue through existential queries', function () {
        var segments = splitSegments(fromDescriptiveString('S.list F.type="Task" N(S.task F.type="Completion")'));
        expect(segments.length).to.equal(2);
    ...
    it('should fork at existential queries', function () {
        var segments = splitSegments(fromDescriptiveString('S.list F.type="Task" N(S.task F.type="Completion") P.assignee F.type="User" S.user F.type="Name"'));
    ...
    it('should skip segments that end in predecessors', function () {
        var segments = splitSegments(fromDescriptiveString('S.list F.type="Task" P.assignee F.type="User" S.user F.type="Name"'));
        expect(segments.length).to.equal(2);
        expect(segments[0].toDescriptiveString()).to.equal('S.list F.type="Task"');
    ...
    it('should accept leading predecessor', function () {
        var segments = splitSegments(fromDescriptiveString('P.assignee F.type="User" S.user F.type="Name"'));
        expect(segments.length).to.equal(1);
